...
\begin{figure}[h]
  ...
  \caption[Die Performanz von Deep-Learning-Ansätzen im Vergleich zu traditionellen Machine-Learning-Modellen.]{Die Performanz von Deep-Learning-Ansätzen im Vergleich zu traditionellen Machine-Learning-Modellen nach \cite[S. 7]{alom_state---art_2019}.}\label{fig:deep-learning-trad-vergleich}
\end{figure}
...
\begin{table}[H]
  ...
  \begin{tabular}{lllll}
  \toprule
  Arbeit & Modell & Acc. ($\%$) & Angaben zu Portierbarkeit & Evaluationsgrundlage \\
         &        &             & oder Ressourcenbedarf \\
  \midrule
  \cite{hemminki_accelerometer-based_2013} & Mehrere Decision Trees & $93,60$ & \cmark & \\
  \cite{reddy_using_2010} & DT und HMM & $93,6$ & \cmark & \\
  \midrule
  \cite{liang_convolutional_2017} & 6-Layer-CNN & $94,48$ & \xmark & \\
  \cite{friedrich_transportation_2019} & LSTM-RNN & $84,22$ & \xmark & \\
  \cite{liono_inferring_2018} & Random Forest & $91$ & & Crowdsignal-Datensatz \\
  \cite{fang_transportation_2016} & SVM & $86$ & & HTC-Datensatz \\
  \midrule
  \cite{friedrich_combining_2020} & LSTM und CNN & & \xmark & SHL-Datensatz \multirow{3}{*}{\cite{gjoreski_versatile_2017}} \\
  \cite{antar_comparative_2018} & Random Forest & $92$ & & \\
  \cite{jeyakumar_deep_2018} & LSTM & $98,1$ & & \\
  \bottomrule
...
% - Vergleich der Accuracy-Metriken von Verkehrsmittelerkennungen
% - Beobachtung: viele der ersten Methoden nutzen GNSS, jetzt eher weniger (?)
% - Analogon zur HAR: Häufig verwechselte Verkehrsmittel?
% - Hemminki: Explizit auf Smartphones lauffähiges Konzept
% - Verwendet traditionelle Machine-Learning-Verfahren, daher auch Vorverarbeitung
% - Erstellt verschiedene Features für die Eingabesignale
% - Ensemble Learning: Mehrere Klassifikatoren
% - Unterschiedlichste Fortbewegungsmittel werden zunächst klassifiziert
% -> Kinematic motion classifier und Stationary classifier: DT mit geringer Tiefe (1, 2)
% -> Accuracy von über 99% (kinematic) bzw. 95% (stationary)
% - Motorized Classifier: DT mit Tiefe 2
...
\begin{tabular}{llllllll}
  \toprule
  {} &   Stehend &   Laufend &       Bus &       Zug &     Metro &      Tram & $\varnothing_{G}$ \\
  \midrule
  Acc. ($\%$) &  93,92 &  98,18 &  92,84 &  92,84 &  91,81 &  91,96 &       93,60 \\
  \bottomrule
\end{tabular}
\label{tab:hemminki-acc}
\caption{Accuracy-Metriken der einzelnen Klassen aus \cite{hemminki_accelerometer-based_2013} und deren gewichteter Durchschnitt $\varnothing_{G}$.}
\end{table}

\begin{table}[h]
\begin{tabular}{llllllllll}
  \toprule
  {} &    Stehen &     Gehen &    Rennen &   Fahrrad &      Auto &      Bus &       Zug &    U-Bahn &  $\varnothing_{G}$ \\
  \midrule
  Acc. ($\%$) &  99,59 &  99,81 &  99,99 &  99,88 &  99,88 &  99,68 &  99,51 &  99,60 &       99,74 \\
  \bottomrule
\end{tabular}
\label{tab:friedrich-acc}
\caption{Accuracy-Metriken der einzelnen Klassen aus \cite{friedrich_combining_2020} und deren gewichteter Durchschnitt $\varnothing_{G}$.}
\end{table}

\section{Tradeoff}

\todo[inline]{Machine-Learning-Arbeiten zur Analyse des Tradeoffs diskutieren}
