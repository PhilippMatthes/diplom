...
\todo[inline]{Dieser Abschnitt befindet sich in der aktiven Bearbeitung.}
...
\begin{figure}[h]
  ...
  \end{subfigure}
  \caption[Machine-Learning-Architekturen der Movebis-Ansätze.]{Machine-Learning-Architekturen der Movebis-Ansätze. Das dreigeteilte RNN-Netzwerk aus \cite{werner_kontinuierliche_2020} wird an der oberen Seite entlang der Zeitreihe eingebunden. \cite{stojanov_continuous_2020} stapelt die erstellten Feature-Fenster in drei Dimensionen (Zeitstempel, Anzahl Features, gestapelte Fenster) für den Input in das CNN.}
  \label{fig:ml-movebis}
\end{figure}

In \Cref{fig:ml-movebis} sind die Architekturen der Machine-Learning-Modelle aus den Movebis-Ansätzen gezeigt. Bei der quantitativen Evaluation der Ansätze wurde insbesondere die Accuracy-Metrik der Netzwerke auf Testdaten bestimmt. Von zentraler Bedeutung für die Beurteilung der Nutzbarkeit auf mobilen Geräten ist jedoch zunächst noch, wie viel Speicher die Netzwerke benötigen. Da \cite{matusek_anwendung_2019, werner_kontinuierliche_2020, stojanov_continuous_2020} keine Aussagen zur genauen Speichergröße ihrer Modelle treffen, wurden diese analog zu den konzeptuellen Beschreibungen mithilfe von Keras\footnote{\url{https://keras.io/} (Abgerufen am 2.6.2021)} implementiert. \Cref{fig:ml-movebis} wurde aus dieser Implementation erzeugt. Um die Speichergröße der Modelle festzustellen, wurden die Modelle nach der Erzeugung im \texttt{.h5}-Format gespeichert und die Größe der erzeugten Modell-Datei gemessen. Bei der Erstellung der Modelle wurde sich so nah wie möglich an den Beschreibungen in \cite{matusek_anwendung_2019, werner_kontinuierliche_2020, stojanov_continuous_2020} orientiert. Unklarheiten wie die genaue Lokalisation der Dropout-Schichten oder auch die endgültige Konfiguration der Input-Schicht des CNN aus \cite{stojanov_continuous_2020} bestanden dennoch. Die ungefähre Größenordnung der Modelle lässt sich dennoch feststellen. Die Ergebnisse dieser Analyse sind in der nachfolgenden Tabelle dargestellt.

\begin{table}[h]
  \begin{tabular}{lll}
  Arbeit & Modellkonfiguration & Modellgröße \\
  \midrule
  \cite{matusek_anwendung_2019} & ??? & 335 kB \\
  \cite{werner_kontinuierliche_2020} & ??? & 623 kB \\
  \cite{stojanov_continuous_2020} & ??? & 1296 kB \\
  \bottomrule
...
% Modellparameter
% Matusek S. 80: 335132 Bytes
% Werner S. 24: 622912 Bytes
% Stojanov S. 76: 1295912 Bytes
