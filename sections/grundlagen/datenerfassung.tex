...
Bei Beschleunigungsdaten des Akzelerometers ist die Erdschwerkraft in den vektorisierten Messdaten integriert \cite{ken_taylor_activity_2011, tundo_correcting_2013}. Befindet sich das Gerät also in Ruhe, so lässt sich der Gravitationsvektor ermitteln. Gleichzeitig wird bei einer Neigungsänderung des Smartphones auch immer eine Änderung des Gravitationsvektors gemessen. Dies erschwert die Interpretation der gemessenen Daten durch ein später hierauf basiertes Erkennungssystem. Für die Behandlung dieses Problems existieren verschiedene Möglichkeiten. Eine Möglichkeit besteht darin, die gemessenen Werte zunächst in ein von der Neigung des Gerätes unabhängiges Bezugskoordinatensystem, das sogenannte Weltkoordinatensystem, zu bringen. Außerdem ist es beispielsweise auch möglich, den Gravitationsvektor mithilfe eines Hochpassfilters (\Cref{sec:signalverarbeitung}) zu entfernen.
...
Ebenfalls erschwerend wirken sich von Gerät zu Gerät unterschiedliche inhärente Messfehler und -parameter der Sensoren aus. Werden die Daten der Sensorik asynchron aufgezeichnet oder nicht bereits durch die Systemschnittstellen synchronisiert, beispielsweise durch verzögerte Aufzeichnung eines Sensors oder durch abweichende Abtastraten, so müssen die Daten nachträglich synchronisiert werden. Außerdem können die Akzelerometerdaten verrauscht sein \cite{dey_accelprint_2014, ravi_deep_2016}, durch unterschiedliche Imperfektionen in der Fertigung und die altersbedingte Degradation der mikroelektronischen Sensorik \cite{matej_andrejasic_mems_2008, constantinescu_capacitive_2013, hillman_manufacturing_2004}. Eine Verkehrsmittelerkennung sollte  daher gegenüber solcher Rauschmuster robust sein \cite{dey_accelprint_2014}. Werden die Rohdaten eines Sensors über einen längeren Zeitraum aufgerechnet, so kann der aggregierte Gesamtwert \textit{abdriften} \cite{takeda_drift_2014}. Dies ist vor allem für Anwendungen relevant, bei denen die Rekonstruktion der Bewegung im Raum notwendig ist. Neben diesen Herausforderungen besteht mit Hinblick auf die Orientierungsdaten ein weiteres Problem. Da das Erdmagnetfeld (ca. $30 \mu T$ bis $60 \mu T$) im Vergleich zu einigen künstlichen Magnetfeldquellen wie Kühlschränken (ca. $5 mT$) relativ schwach ist, kann das Magnetometer leicht gestört oder dekalibriert werden \cite{moreno-torres_evaluation_2013, schirmer_smartphone_2016, zhang_preliminary_2012}, was zu signifikanten Fehlern führen kann \cite{kunze_can_2010}.

\subsubsection{Signalverarbeitungsmethoden}\label{sec:signalverarbeitung}

\begin{figure}[h]
\includegraphics[width=0.75\linewidth, bb=0 0 733 284]{frequenzpassfilter.pdf}
...
\begin{figure}[h]
\includegraphics[width=0.75\linewidth, bb=0 0 734 339]{fft.pdf}
...
Ein weiteres Signalverarbeitungsverfahren ist die \textit{Fourier-Transformation}\footnote{\url{https://de.wikipedia.org/wiki/Fourier-Transformation} (Abgerufen am 8.4.2021)}. Die Fourier-Transformation ist ein mathematisches Verfahren, um zusätzlich zum Zeitlinienverlauf der Signale deren Frequenzspektrum zu ermitteln. Mithilfe einer \textit{Short Time Fourier Transformation} (\acrshort{stft}) können Spektrogramme erstellt werden, in \Cref{fig:fft} ist eine \textit{Fast Fourier Transformation} (\acrshort{fft}) gezeigt. Anhand des ermittelten Frequenzspektrums lässt sich sofort ablesen, welche Frequenzen besonders häufig oder sehr selten in einem Frequenzspektrum vorkommen, sowie die gemessene Maximal- und Minimalfrequenz. Dies ist nicht nur relevant für die explorative Datenanalyse, sondern kann auch als zusätzliche Informationsquelle für das Machine-Learning-Modell dienen.

\subsubsection{Sensorfusion}

Die Sensordaten können für eine Weiterverarbeitung einzeln betrachtet werden \cite{gudur_activeharnet_2019, nutter_design_2018}, dies genügt bereits für die Rekonstruktion von komplexen Bewegungsabläufen \cite{cai_touchlogger_2011, marquardt_spiphone_2011, aviv_practicality_2012}. Alternativ können die Sensordaten miteinander \textit{fusioniert} werden \cite{ravi_deep_2016, abdulla_measuring_2013, kunze_can_2010}, um eine abstraktere Repräsentation der Bewegung des Gerätes zu erhalten. Dies wird in der Regel durch Filter wie das Madgwick-Filter \cite{madgwick_estimation_2011} umgesetzt. Speziell bei Beschleunigungsdaten kann hierdurch zum Beispiel auch der inkludierte Gravitationsvektor eliminiert werden \cite{nutter_design_2018, tundo_correcting_2013}. Außerdem kann die Orientierung und Auslenkung des Gerätes in Relation zum Weltkoordinatensystem rekonstruiert werden.
...
Ein Koordinatensystem, welches mithilfe mathematischer Filter die Lage des Gerätes als Weltkoordinatensystem rekonstruiert, wird auch als \textit{Attitude and Heading Reference System (\acrshort{ahrs})} bezeichnet.

\subsubsection{Synchronisierung und Alignment}

Sofern eine Synchronisierung von Sensordaten notwendig ist, kann ein Up- oder Downsampling mithilfe einer Interpolation der Datenpunkte durchgeführt werden \cite{matusek_anwendung_2019, werner_kontinuierliche_2020, stojanov_continuous_2020}.
...
\Cref{fig:desync-solution} zeigt das in \cite{matusek_anwendung_2019} beschriebene Verfahren schematisch. Die mit Zeitstempeln versehenen Datenpunkte werden in ein festes Abtastraster gebracht. Dies gelingt über zwei primäre Schritte. Am zeitlichen Rand der Messung werden zunächst Datenpunkte verworfen, zu denen kein Wertepaar aus allen drei Sensoren gebildet werden kann. Ein Wertepaar lässt sich erst dann bilden, wenn alle Sensoren mindestens einen Wert erfasst haben, am Ende der Messung genauso umgekehrt. Das so gefundene Fenster wird anschließend im zweiten Schritt anhand der vorliegenden Datenpunkte pro Sensormessung interpoliert, um den diskreten Punktdatensatz in eine kontinuierliche Näherungsfunktion zu überführen. Die Abtastzeitpunkte $t_s ... t_{s+n}$ (in \Cref{fig:desync-solution} als $\otimes$ gekennzeichnet) können anschließend für alle drei Messverläufe beliebig gewählt und errechnet werden.

\begin{figure}[h]
\includegraphics[width=0.75\linewidth, bb=0 0 505 175]{interpolation.pdf}
...
\begin{table}[h]
  \begin{tabular}{ll}
    ...
    Gyrosensor & Winkelauslenkungen, negativ und positiv, in $\frac{rad}{s}$ \\
