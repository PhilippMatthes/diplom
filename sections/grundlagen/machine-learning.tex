...
Die zentrale Idee hinter dem Training von Machine-Learning-Modellen durch Supervised Learning ist die Definition einer zu minimierenden Kostenfunktion. Eine typische Kostenfunktion ist der \textit{Root-Mean-Square Error}\footnote{\url{https://en.wikipedia.org/wiki/Root-mean-square_deviation} (Abgerufen am 6.5.2021)} (RMSE). Die Kostenfunktion soll beschreiben, wie stark das aktuelle Modell (anhand dessen Vorhersage) von dem Zielwert abweicht. Die Kostenfunktion wird minimiert, indem die mathematischen Parameter des Machine-Learning-Modells iterativ in Abhängigkeit von ihrem Beitrag zum Fehler angepasst werden.

\subsubsection{Unsupervised Learning}

Das Unsupervised Learning ähnelt dem Supervised Learning, denn auch hier ist das Ziel, die Kosten eines Modells zu minimieren. Während jedoch beim Supervised Learning die Zielwerte bekannt sein müssen, um die Vorhersage mit dem tatsächlichen Wert zu vergleichen, erfolgt das Unsupervised Learning ohne diese Zielwerte. Beim Unsupervised Learning sollen Muster innerhalb des Datensatzes erkannt werden. Das Verfahren wird daher typischerweise für die Segmentierung von Daten angewandt. Das Modell entwickelt hierbei durch Anpassung der Modellparameter Vermutungen über die Muster eines Datensatzes. Die Kosten bestimmen sich daraus, inwiefern sich diese vermuteten Muster in neuen Daten wieder abbilden. Auch hier lassen sich hierüber die Parameter des Machine-Learning-Modells anpassen.

\subsubsection{Gradientenabstiegsverfahren}

Die Minimierung der Kostenfunktion beim Supervised Learning und Unsupervised Learning ist ein mathematisches Optimierungsproblem. Ziel ist es, die Parameter des Machine-Learning-Modells so anzupassen, dass die Kosten minimal werden. Bei einem kleinen Zustandsraum (aufgespannt durch die Modellparameter) ist dies durch lineare Gleichungssysteme realisierbar. Bei vielen Modellparametern eignen sich wegen der Zunehmenden Komplexität der Berechnung sogenannte Gradientenabstiegsverfahren. Diesen zugrundeliegend ist die Idee, dass sich bei der numerischen Variation der einzelnen Modellparameter die Kosten vergrößern oder verkleinern. Dieser lineare Zusammenhang kann als Gradient betrachtet werden. Beim Gradientenabstiegsverfahren werden die Modellparameter iterativ so angepasst, dass sich zur Verringerung der Kosten entlang des steilsten Gefälles am Gradient nach unten bewegt wird.

\todo{Symbolbild einfügen}

\noindent Die Intensität (Schrittweite auf dem Gradienten), mit der die Modellparameter bei jedem neuen Datenpunkt angepasst werden, wird als \textit{Learning Rate} bezeichnet. Die Learning Rate ist ein wichtiger Hyperparameter von Machine-Learning-Systemen. Ein zentrales Problem von Gradientenabstiegsverfahren ist, dass Gradienten neben einem zu erreichenden globalem Minimum auch zahlreiche lokale Minima besitzen können. Ist die Learning Rate zu klein, so lernt das Modell verhältnismäßig langsam und läuft gleichzeitig Gefahr, in ein lokales Minimum des Gradienten zu fallen. Ist die Learning Rate jedoch zu hoch, besteht das Risiko, dass aus Minima \enquote{herausgesprungen} wird. Zentraler Teil des Trainings von Machine-Learning-Modellen ist es daher, das Lernverhalten zu beobachten und ggf. ein \textit{Hyperparameter Tuning} durchzuführen. Diese Betrachtung ist jedoch vereinfacht und dient nur zur Veranschaulichung des Funktionsprinzips. Bei erweiterten Optimierungsverfahren wie dem \textit{ADAM-Optimizer} wird die Learning Rate beispielsweise pro Modellparameter individuell angepasst.

\subsubsection{Overfitting, Underfitting und Early Stopping}

% early stopping erklären

% Lösung: Iterative Verbesserung eines Modells = Rechenaufwändige Anpassung der Modellparameter
% Inferenz: Bestimmung der Ausgabe anhand des Inputs, ohne Parameteranpassung (Weniger rechenaufwändig, da Parameter nicht angepasst werden müssen)
% Verschiedene Lerntypen: Supervised (Ausreichend gelabelte Daten), Unsupervised (Keine gelabelten Daten), Reinforcement Learning (Evolutionär)
% Implementation: Iteratives Gradientenabstiegsverfahren, Backpropagation
% Probleme: Overfitting, Underfitting, False Pattern Recognition, ...
% Lösungen: Learning Rate Tuning, Datenvorverarbeitung, ..
% Erweiterte Verfahren: Transfer Learning (Wenige gelabelte Daten), Semi-Supervised (Wenige gelabelte Daten), Ensemble Learning (Mehrere Modelle zusammen).

% Architekturen
% Decision Trees und Random Forests (Matusek)
% SVM (Matusek)
% Deep Learning
...
% Typische Probleme von Machine-Learning-Modellen
