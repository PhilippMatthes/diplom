...
Die Sussex-Huawei-Locomotion-Challenge, kurz SHL-Challenge, ist ein jährlicher Wettbewerb, bei dem Forschungsteams Verkehrsmittelklassifikationen mit Fokus auf eine bestmögliche Accuracy entwickeln. Im Rahmen des Wettbewerbs werden mehrere Datensätze zur Verfügung gestellt, mit unterschiedlichen Strukturen. Während der STADTRADELN-Datensatz nicht öffentlich zur Verfügung steht, bietet der SHL-Datensatz die Möglichkeit des Vergleichs mit anderen Forschungsansätzen, welche im Rahmen der SHL-Challenge erstellt wurden \cite{wang_summary_2020}. Das aus dem ursprünglichen Movebis-Ansatz \cite{werner_kontinuierliche_2020} entstandene System wurde in \cite{grubitzsch_ai-based_2021} erst kürzlich ebenfalls auf einem SHL-Datensatz getestet, wodurch zusätzlich auch ein Vergleich mit einem Movebis-Ansatz möglich ist.

Die bei der SHL-Challenge veröffentlichten SHL-Datensätze enthalten mehrere Pakete, jeweils aufgezeichnet in einer unterschiedlichen Position am Körper (\textit{Torso}, \textit{Bag}, \textit{Hips} und \textit{Hand}). Insgesamt wurden die Daten von drei Personen aufgezeichnet.
...
\caption{Verteilung der FFT-Frequenzen von $\left| ACC \right|$, $\left| MAG \right|$ und $\left| GYR \right|$ anhand der Klassen im SHL-Hand-Datensatz.}\label{fig:shl-sensor-fft}
\end{figure}
...
Zur Selektion der Basisarchitektur wird auf die zwei Feature-Schnittstellen der Vorverarbeitungspipeline zurückgegriffen. Es werden typische Metaarchitekturen und Modellkonfigurationen aus verwandten Arbeiten, vor allem den Movebis-Ansätzen, extrahiert und auf dem SHL-Datensatz prototypisch trainiert. Die Selektion erfolgt argumentativ auf Grundlage der so ermittelten Parameter, der Ergebnisse verwandter Arbeiten und unter Berücksichtigung des Potenzials für die nachfolgenden Selektionsschritte.
...
Mit Hinblick auf die Movebis-Architekturen werden zunächst Basisarchitekturen getestet, welche aus diesen Arbeiten entspringen. Hierbei ist zunächst die LSTM-Basisarchitektur relevant, von der im Movebis-Ansatz \cite{werner_kontinuierliche_2020} eine Variation verwendet wurde. Als LSTM wird ein einschichtiges Modell mit Dropout-Regularisierung auf direkten Zeitliniendaten getestet. Außerdem wird die CNN-Basisarchitektur, wie sie in \cite{stojanov_continuous_2020} verwendet wurde, getestet. Gewählt wird ein einfaches CNN mit ReLU-Aktivierungsfunktion und Regularisierung durch Batch Normalization. Das CNN besteht aus drei Convolution-Schichten wie in \cite{stojanov_continuous_2020}, analog zu \cite{wang_time_2016} wird jedoch kein intermediäres Max-Pooling durchgeführt, sondern ein Global Average Pooling zum Schluss der drei Convolution-Schichten. Auf Grundlage der CNN-Basisarchitektur aus \cite{stojanov_continuous_2020} wird eine weitere Basisarchitektur verwendet, das ResNet. Dieses substituiert die drei Convolution-Schichten wie im Movebis-Ansatz \cite{stojanov_continuous_2020} durch drei Convolution-Blöcke mit je drei Convolution-Schichten, welche untereinander durch Shortcut-Verbindungen ein tieferes Modell ermöglichen sollen, als es mit einem herkömmlichen CNN möglich wäre \cite{he_deep_2015, ismail_fawaz_deep_2019}. Da aus der Forschungsanalyse hervorgegangen ist, dass tiefere Modelle bei der Verkehrsmittelerkennung bessere Ergebnisse erzeugen, könnte somit eine bessere Analyse der Tiefe erfolgen. Zu den bereits selektierten CNN-Architekturen und einer LSTM-Architektur werden außerdem noch weitere neuartige Architekturen hinzugezogen. Hierzu gehört ein CNN-LSTM. Dies ist ein LSTM, welches einem einschichtigen CNN mit ReLU-Aktivierungsfunktion zur Kodierung der Eingabewerte nachgeschaltet ist. Das CNN übernimmt hierbei also die dedizierte Aufgabe der Feature-Codierung. Alle CNN- und LSTM-Modelle verwenden eine Dense-Schicht als Ausgabeschicht, welche mithilfe der Softmax-Aktivierungsfunktion die kategorische Klassifikation übernimmt. Als neuartiger Ansatz wird außerdem ein Transformer-Netzwerk getestet, welches auf einem neuartigen Verfahren des Lernens basiert, welches als \textit{Attention}-based Learning bezeichnet wird \cite{vaswani_attention_2017, theodoros_ntakouris_keras_2021}.

\subsubsection{Hyperparameter des prototypischen Trainingsprozesses}

Beim Training der gewählten Deep-Learning-Modelle werden folgende Regeln angewandt. Die Modelle werden in die zuvor erläuterte Vorverarbeitungspipeline eingebunden und auf Batches mit einer Größe von 32 Samples (\textit{Batch-Size}) durch Supervised Learning trainiert, die Samples werden einem Shuffling unterzogen. Die Batch-Size von 32 hat sich für die gewählten Modelle auf dem Testsystem als performantere Konfiguration gegenüber größeren Batch-Sizes von 64 oder 128 erwiesen. Die Validierung des Modells geschieht auf einem separierten Datensatz. Als Variante des Gradientenabstiegsverfahrens wird der Adam-Optimizer genutzt. Als Alternativen stehen unter anderem der SGD-Optimizer und der RMSprop-Optimizer zur Verfügung. Der Adam-Optimizer zeigte im prototypischen Trainingsprozess gegenüber den anderen Optimierungsverfahren keine signifikanten Vor- und Nachteile, bis auf eine marginal schnellere Konvergenz. Als zugrundeliegende Verlustfunktion wird die \textit{Sparse Categorical Cross Entropy} verwendet, ein Derivat der Kreuzentropie, welche die Notwendigkeit eines One-Hot-Encodings der Labels beim Training der Modelle erübrigt. Da die Labels des SHL-Datensatzes in Form von Integer-Encodings vorliegen, muss somit keine weitere Vorverarbeitung auf den Labels erfolgen. Auf Grundlage der Validierungsmetriken wird Early Stopping verwendet, sowie eine automatisierte Reduktion der Learning Rate bei einer stagnierenden Verbesserung der Accuracy auf den Validierungsdaten.
...
Darüber hinaus können auch die eindimensionalen Convolution-Schichten der Basisarchitektur auf zweidimensionale Convolution-Schichten übertragen werden. Dies wird in Betracht gezogen, da ResNets ursprünglich nicht für die Verarbeitung von 1D-Sequenzen entwickelt wurden, sondern zur Klassifikation von Bildern in Form von zweidimensionalen RGB- bzw. BGR-Pixeldaten. Hierfür wird eine portierbare STFT-Transformationsschicht (\textit{Short-Time-Fourier-Transform}) vorgeschaltet, deren Ausgabe analog zu \Cref{fig:stft-layer-output} als Eingabe für diese Architekturen dienen können. Dies fundiert auf den Beobachtungen, dass eine Fourier-Transformation wertvolle Informationen über den Signalverlauf entschlüsseln kann. Im Zuge dessen könnten auch auf dem Imagenet-Datensatz aus \cite{deng_imagenet_2009} vortrainierte ResNet-Modelle (ResNet50, ResNet101, ResNet152) in die Rastersuche einbezogen werden, um Transfer Learning als konzeptuellen Bestandteil mit einzubeziehen und potenziell von den bereits erlernten visuellen Features zu profitieren.
...
Als Datensatz wird der SHL-Forschungsdatensatz akquiriert. Auf diesem Datensatz wurden traditionelle Modelle und Deep-Learning-Modelle prototypisch getestet, unter Einbeziehung der Basisarchitekturen aus den Movebis-Ansätzen \cite{werner_kontinuierliche_2020,stojanov_continuous_2020,matusek_anwendung_2019}, insbesondere DT, MLP, LSTM und CNN. Dies inkludiert individuell angepasste Datenvorverarbeitungspipelines, welche empirisch durch eine Datensatzanalyse konfiguriert wurden. Dazu gehört, dass neben einem speichereffizienten Lademechanismus die im Datensatz enthaltenen Klassen durch SMOTE oder die Gewichtung der Modell-Verlustfunktion wie in \cite{werner_kontinuierliche_2020} gleichgewichtet werden. Außerdem werden die Datenpunkte analog zu \cite{werner_kontinuierliche_2020} in eine annähernd symmetrische Repräsentation durch Yeo-Johnson-Transformation überführt. Das beste empirisch gefundene Basismodell ist eine Variante der ResNet-CNN-Architektur, welche eine Erweiterung der CNN-Basisarchitektur aus \cite{stojanov_continuous_2020} darstellt. Zur Elimination von Verschiebungsfaktoren wie der Gravitation werden die Sensorsignale des Akzelerometers, des Magnetometers und des Gyrosensors bei diesem Modell in Zeitlinien des jeweiligen Vektorbetrags überführt, ohne Anwendung eines filterbasierten AHRS, wie es in \cite{matusek_anwendung_2019} vorgestellt wurde. Shallow Features und Non-Shallow Features wurden bei der Selektion des Basismodells ebenfalls mit einbezogen. Anhand von vorläufigen Tests und Betrachtungen der Portierbarkeit und der Ressourcenintensität wurden Modelle auf diesen Features nicht weiter verfolgt.
