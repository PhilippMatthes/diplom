...
Vor diesem Hintergrund scheint es attraktiv, statt Shallow Features händisch zu erstellen, die Zeitliniendaten direkt durch Deep-Learning-Modelle klassifizieren zu lassen. Dessen Plausibilität wurde durch verwandte Arbeiten gezeigt, und der Aufwand der Portierung würde um ein Vielfaches reduziert. Um dies näher zu betrachten, werden verschiedene Deep-Learning-Modelle, wieder prototypisch, jedoch nun direkt auf den Zeitliniendaten getestet. Insgesamt wurden die folgenden Modelle durch Integration in die zuvor erläuterte Vorverarbeitungspipeline getestet:

\begin{itemize}
\item Ein CNN mit ReLU-Aktivierungsfunktion und Regularisierung durch Batch Normalization nach \cite{wang_time_2016}
\item Ein CNN mit ReLU-Aktivierungsfunktion und Regularisierung durch Batch Normalization, welches durch seine besondere Tiefe und durch Shortcuts zwischen tiefen Schichten gekennzeichnet ist (\textit{ResNet}), nach \cite{he_deep_2015, ismail_fawaz_deep_2019}
...
\item Ein \textit{attention}\footnote{\url{https://en.wikipedia.org/wiki/Attention_(machine_learning)} (Abgerufen am 16.8.2021)}-basiertes Transformer-Netzwerk nach \cite{vaswani_attention_2017, theodoros_ntakouris_keras_2021}
\end{itemize}

\paragraph{Hyperparameter des prototypischen Trainingsprozesses:} Beim Training der Deep-Learning-Modelle werden folgende Regeln angewandt. Die Modelle werden in die zuvor erläuterte Vorverarbeitungspipeline eingebunden und auf Batches mit einer Größe von 32 Samples (\textit{Batch-Size}) durch Supervised Learning trainiert, die Samples werden einem Shuffling unterzogen. Die Batch-Size von 32 hat sich für die gewählten Modelle auf dem Testsystem als performant gegenüber größeren Batch-Sizes von 64 oder 128 erwiesen. Die Validierung des Modells geschieht auf einem separierten Datensatz. Als Variante des Gradientenabstiegsverfahren wird der Adam-Optimizer genutzt. Der Adam-Optimizer zeigte gegenüber anderen Optimierungsverfahren eine schnellere Konvergenz. Als zugrundeliegende Verlustfunktion wird die \textit{Sparse Categorical Cross Entropy} verwendet, ein Derivat der Kreuzentropie, welche die Notwendigkeit eines One-Hot-Encodings der Labels erübrigt. Auf Grundlage der Validierungsmetriken wird Early Stopping verwendet, sowie eine automatisierte Reduktion der Learning Rate bei einer stagnierenden Verbesserung der Accuracy auf den Validierungsdaten.
...
\begin{figure}[h]
  \includegraphics[width=\linewidth, bb=0 0 482 190]{hypermodell.pdf}
  \caption{Hyperparameter-Metamodell der ResNet-Basisarchitektur.}\label{fig:hypermodell}
\end{figure}

Ziel der Definition eines Hyperparameter-Metamodells ist die parametrisierte Variation der initialen Basisarchitektur. Die Parameter sind hierbei ausgewählte Hyperparameter der Basisarchitektur. Dies können Hyperparameter im engeren Sinn wie die Learning Rate oder die Anzahl der tiefen Schichten sein, oder auch abstraktere Variationsmöglichkeiten wie die Art der Aktivierungsfunktion oder die Selektion einer vordefinierten Gesamtarchitektur auf Grundlage der Eigenschaften der Basisarchitektur. \Cref{fig:hypermodell} zeigt das für die ResNet-Basisarchitektur erstellte Hyperparameter-Metamodell. Das bei der Selektion der Basisarchitektur getestete Modell klassifiziert direkt auf den Zeitliniendaten, hierbei können verschiedene Regularisierungstechniken variiert werden (Dropout, Batch-Normalization, Kernel-Regularisierung durch L1/L2), die Aktivierungsfunktion (ReLU, Leaky ReLU und deren $\alpha$-Parameter) und die generelle Größe des Netzwerkes (Anzahl tiefer Schichten, Größe der Schichten).
...
Gleichzeitig können auch Gesamtarchitekturen des ResNet variiert werden, welche eine möglicherweise geeignete Konfiguration der Hyperparameter bereits inkludieren, abhängig vom ursprünglichen Anwendungsfall. Hierbei eignen sich die Architekturen aus \cite{he_deep_2015} (ResNet50, ResNet101, ResNet152) und deren Weiterentwicklung aus \cite{he_identity_2016} (ResNet50V2, ResNet101V2, ResNet152V2), welche auf Bilddaten sehr gute Ergebnisse erzielen. Im Umkehrschluss bestehen diese Architekturen aus 2D-Convolution-Schichten für die Verarbeitung der dreidimensionalen RGB- bzw. BGR-Pixeldaten. Um diese Netzwerke trotzdem einbeziehen zu können, wird diesen eine portierbare STFT-Transformationsschicht (\textit{Short-Time-Fourier-Transform}) vorgeschaltet, deren Ausgabe analog zu \Cref{fig:stft-layer-output} als Eingabe für diese Architekturen dienen können. Im Zuge dessen können auch auf dem Imagenet-Datensatz aus \cite{deng_imagenet_2009} vortrainierte Modelle in die Rastersuche einbezogen werden, um Transfer Learning als konzeptuellen Bestandteil mit einzubeziehen und potenziell von den bereits erlernten visuellen Features zu profitieren.

\subsection{Top-N-Selektion durch Metamodell-Optimierung}
