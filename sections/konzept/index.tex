...
\subsection{Selektion einer Skalierungsmethode}\label{sec:skalierungsmethode}
...
\subsubsection{Gewichtung des Datensatzes}
...
\section{Algorithmus zur Tradeoff-Analyse}

Ziel der Tradeoff-Analyse ist die Ermittlung eines Pareto-optimalen Modells bezüglich der Relation zwischen Ressourcenverbrauch und Ergebnisqualität. Die Konfiguration des Modells ist hierbei maßgeblich von drei Dimensionen abhängig. Bereits durch die Selektion einer Basisarchitektur spannt sich eine große Menge von möglichen Modellen auf. Diese wiederum können in ihren Hyperparametern und applizierbaren Optimierungsstrategien variiert werden. Unmöglich können alle Modelle in dieser Arbeit getestet werden, die kombinatorische Explosion ist offensichtlich. Die in dieser Arbeit gewählte Lösung hierfür ist die schrittweise Exploration jeder der genannten Dimensionen, wobei nach jedem Schritt das vielversprechendste Modell selektiert wird.
...
\begin{figure}[h]
  ...
  \caption{Erreichte Accuracy-Werte und F1-Scores traditioneller Machine-Learning-Modelle auf dem SHL-Datensatz mit 24 Features.}\label{fig:shl-traditional-models}
\end{figure}
