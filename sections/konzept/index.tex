...
Ziel der Konzepts ist die Ermittlung eines Pareto-optimalen Modells bezüglich der Relation zwischen Ressourcenverbrauch und Ergebnisqualität auf Smartphones, auf Grundlage der Kernkonzepte der Movebis-Ansätze. Zunächst muss hierfür ein Datensatz akquiriert werden, welcher zum Training und zur Evaluation genutzt werden kann. Im Rahmen der Forschungsanalyse konnte kein konkreter Modellansatz isoliert werden, der übergreifend die besten Ergebnisse erzielt. Es existieren stattdessen mehrere mögliche Herangehensweisen für die Konfiguration einer Datenvorverarbeitungspipeline und des nachfolgenden Modells. Die Konfiguration ist hierbei maßgeblich von drei Dimensionen abhängig. Bereits durch die initiale Selektion einer Basisarchitektur spannt sich eine große Menge von möglichen Modellen auf, darunter auch die Basisarchitekturen \acrshort{dt} und \acrshort{mlp} aus \cite{matusek_anwendung_2019}, \acrshort{lstm} aus \cite{werner_kontinuierliche_2020} und \acrshort{cnn} aus \cite{stojanov_continuous_2020}. Insbesondere für traditionelle Modelle muss ein extensives Feature-Engineering durchgeführt werden. Die so entstehenden Modellvarianten wiederum können in ihren Hyperparametern und applizierbaren Optimierungsstrategien variiert werden. Damit ist es zumindest im Rahmen dieser Arbeit nicht möglich, alle Kombinationen und Variationen auszutesten. Die zu testenden Modelle sollen stattdessen auf Grundlage empirischer Entscheidungen selektiert werden. Dies beinhaltet eine schrittweise Breitensuche jeder der genannten Dimensionen, wobei nach jedem Schritt das vielversprechendste Modell heuristisch selektiert wird.
...
Die Zeitliniendaten des \acrshort{shl}-Datensatzes können direkt geladen und anschließend für Training und Test genutzt werden. Unter anderem sind hierbei jedoch viele zusätzliche Zeitlinien-Informationen enthalten, welche bezüglich des potenziellen Informationszugewinns zu prüfen sind. Daher werden zwei mögliche Berechnungen von direkten Zeitlinien-Features untersucht. Zum einen die Verwendung aller bereitgestellten Informationen mit der damit verbundenen relativen Erhöhung der Operationen zur Berechnung (energieintensiver) und zum anderen die Verwendung einer Selektion aus den möglichen Informationen.

Als Grundlage der selektiven Berechnung sollen die im Datensatz enthaltenen und auch von den Movebis-Ansätzen verwendeten Akzelerometer-, Magnetometer- und Gyrosensordaten verwendet werden. Durch die Berechnung des punktweisen Vektorbetrags über diese Zeitlinien lässt sich die Dimensionalität des Inputs für das ML-Modell zusammen mit den notwendigen Operationen reduzieren. Die Gleichungen sind $\left| ACC \right| = \sqrt{ACC_x^2 + ACC_y^2 + ACC_z^2}$, $\left| MAG \right| = \sqrt{MAG_x^2 + MAG_y^2 + MAG_z^2}$ und $\left| GYR \right| = \sqrt{GYR_x^2 + GYR_y^2 + GYR_z^2}$. Die statistischen Auswirkungen dieser Berechnung sind in den nachfolgenden Grafiken dargestellt.

\begin{figure}[h]
\includegraphics[width=\linewidth, bb=0 0 1396 386]{shl/akzelerometer-boxplot.pdf}
\caption{Statistische Verteilung der Sensorkomponenten im Akzelerometer-Signal anhand der Klassen im \acrshort{shl}-Hand-Datensatz.}\label{fig:shl-akzelerometer-boxplot}
\end{figure}

In \Cref{fig:shl-akzelerometer-boxplot} wird die in den Grundlagen erläuterte Beobachtung deutlich, dass der Gravitationsvektor in den Teilachsen einen einheitlichen Verschiebungsfaktor darstellt. Durch Berechnung des Vektorbetrags zentriert sich die Verteilung deutlich vertikal um die Erdbeschleunigung, welche bei ungefähr $9,81 \frac{m}{s^2}$ liegt. Durch Subtraktion dieser Beschleunigung, beispielsweise durch Standardisierung, ließe sich der Gravitationsvektor zumindest aus dem Vektorbetrag effizient und verlustfrei entfernen, im Unterschied zum schätzungsbedingt fehlerbehafteten und berechnungsintensiveren \acrshort{ahrs}-Verfahren. Es liegt nahe, dass Informationen aus dem \acrshort{shl}-Datensatz wie der aktuelle Gravitationsvektor ($GRA_{x,y,z}$), die lineare Beschleunigung ($LACC_{x,y,z}$) und die Ausrichtung des Geräts in Quaternionen ($ORI_{w,x,y,z}$) durch die Elimination des Gravitationsvektors für die Klassifikation an Bedeutung verlieren, da das Modell den gravitativen Faktor gegebenenfalls nicht selbstständig eliminieren muss. Umgekehrt lässt sich vermuten, dass ein geeignetes ML-Modell quantitative Zusammenhänge zwischen $GRA_{x,y,z}$, $ORI_{w, x,y,z}$, $LACC_{x,y,z}$ und $MAG_{x,y,z}$, $ACC_{x,y,z}$, $GYR_{x,y,z}$ erlernen und kombinieren kann. Das Risiko des Overfittings ist bei Letzterem jedoch vermutlich höher, denn die Transformation in $\left| ACC \right|$, $\left| MAG \right|$ und $\left| GYR \right|$ kann als Regularisierung der Inputs betrachtet werden, da überflüssige Informationen wie die lineare Verschiebung durch Gravitation entfernt werden.
...
\caption{Statistische Verteilung der Sensorkomponenten im Magnetometer-Signal anhand der Klassen im \acrshort{shl}-Hand-Datensatz.}\label{fig:shl-magnetometer-boxplot}
\end{figure}
...
\caption{Statistische Verteilung der Signalstärken von $\left| ACC \right|$, $\left| MAG \right|$ und $\left| GYR \right|$ im \acrshort{shl}-Hand-Datensatz.}\label{fig:shl-sensor-histograms}
\end{figure}

Die Auswahl der Skalierungsmethode für die Eingaben ist für einige Modelle wichtig, um eine bestmögliche Ergebnisqualität zu erzielen. Hierzu wird die statistische Verteilung der Eingaben näher betrachtet. In \Cref{fig:shl-sensor-histograms} sind die Verteilungen von $\left| ACC \right|$, $\left| MAG \right|$ und $\left| GYR \right|$ für den \acrshort{shl}-Hand-Datensatz illustriert, die Prinzipien lassen sich jedoch auch auf alle partiellen Zeitlinien $GRA_{x,y,z}$, $ORI_{w, x,y,z}$, $LACC_{x,y,z}$, $MAG_{x,y,z}$, $ACC_{x,y,z}$ und $GYR_{x,y,z}$ ableiten.
...
\Cref{fig:shl-scaler-histograms} zeigt die bereits erläuterte Standardisierung. Die Standardisierung verändert die Verteilung der Daten nicht morphologisch, stattdessen werden die Daten mit Erhaltung von Ausreißern um den horizontalen Nullpunkt skaliert. In manchen Fällen\footnote{Vor allem neuronale Netzwerke profitieren von einer standardisierten und symmetrisierten Repräsentation der Inputs.} ist es zur Vorverarbeitung sinnvoll, die Daten zusätzlich morphologisch um den horizontalen Nullpunkt symmetrisch anzugleichen, indem spezielle Power-Transformationen angewandt werden. Der Movebis-Ansatz \cite{werner_kontinuierliche_2020} nutzt die verlustfreie Yeo-Johnson-Transformation, welche in \Cref{fig:shl-scaler-histograms} gezeigt ist. Die Yeo-Johnson-Transformation ist auf einem Datenpunkt $x_i$ wie folgt definiert.
...
\begin{figure}[h]
  ...
  \caption{Resultate eines 1D Deep Convolutional Autoencoders auf Akzelerometerdaten des \acrshort{shl}-Testdatensatzes.}\label{fig:autoencoder}
\end{figure}

In \Cref{fig:autoencoder} sind die Resultate eines aus dem Prototyping entstandenen Autoencoders gezeigt. Der Autoencoder besteht aus mehreren, konzentrisch kleiner werdenden Schichten von eindimensionalen Convolution-Schichten mit zwischengeschalteten Dropout-Schichten zur Regularisierung. Diese Architektur eignet sich für die komplexe Repräsentation von Zeitlinien wesentlich besser als \acrshort{lstm}-Schichten oder Dense-Schichten \cite{martinez_autoencoders_2019}. Auch durch mehrfache Variation der Hyperparameter des Autoencoders konnte jedoch keine hinreichende Generalisierungsfähigkeit erreicht werden. Diese Ergebnisse werden in \Cref{fig:autoencoder} deutlich. Die Kurvenverläufe können teils rekonstruiert werden, bei einigen Samples jedoch kann keine hinreichende interne Repräsentation gefunden werden und die Rekonstruktion fällt zurück auf ein wiederkehrendes Muster (zweite Spalte von rechts). Zu beobachten ist auch, dass der Autoencoder stets zur Minimierung der Kosten bestrebt ist, die Ausgangszeitlinie ähnlich wie beim Moving-Average-Verfahren direkt abzubilden, statt eine abstraktere Repräsentation zu etablieren. Wird die Dimensionalität des Encoders zu groß gewählt, so kann beobachtet werden, dass das Modell lediglich die Eingabedaten in transformierter Form im Encoding abspeichert. Dies würde jedoch vermutlich keinen Mehrwert für ein nachgeschaltetes Klassifikationsmodell gegenüber den reinen Eingangsdaten bieten.
...
\Cref{fig:vorverarbeitungspipeline} zeigt diese Schritte. Im Anschluss hieran werden die Daten entweder für das Training von traditionellen Modellen in Shallow Features überführt und durch \acrshort{smote} gewichtet, oder für das Training von Deep-Learning-Modellen als direkte Zeitliniendaten bereitgestellt, zusammen mit einer Auflistung der Klassenverteilungen zur Gewichtung der Kostenfunktion beim Training.

\section{Empirische Selektion einer Basisarchitektur}

Zur Selektion der Basisarchitektur wird auf die zwei Feature-Schnittstellen der Vorverarbeitungspipeline zurückgegriffen. Es werden typische Metaarchitekturen und Modellkonfigurationen aus verwandten Arbeiten, vor allem den Movebis-Ansätzen, extrahiert und auf dem \acrshort{shl}-Datensatz prototypisch trainiert. Die Selektion erfolgt argumentativ auf Grundlage der so ermittelten Parameter, der Ergebnisse verwandter Arbeiten und unter Berücksichtigung des Potenzials für die nachfolgenden Selektionsschritte.

\subsection{Test von traditionellen Modellen}

Die Forschunganalyse hat ergeben, dass traditionelle Modelle bei der Verkehrsmittelerkennung gegenüber Deep-Learning-Ansätzen nicht zu vernachlässigen sind. Gleichzeitig gelten Deep-Learning-Ansätze speziell bei der Verfügbarkeit von großen Datensätzen wie dem \acrshort{shl}-Datensatz als die vielversprechenderen Ansätze. Die Ergebnisqualität der traditionellen Modelle ist außerdem maßgeblich von der Auswahl und Anzahl der berechneten Shallow Features abhängig, wobei deren Berechnung zur Energieintensität beiträgt. Mit Betrachtung der von Forschungsteams erzielten Ergebnisse bei der \acrshort{shl}-Challenge aus 2020 zeigt sich, dass der beste Deep-Learning-Ansatz auf Zeitliniendaten mit einem F1-Score von $88,5\%$ dem besten traditionellen Ansatz ($77,9\%$) mit Abstand überlegen ist \cite{wang_summary_2020}.

\begin{figure}[h]
  \includegraphics[width=\linewidth, bb=0 0 1063 262]{shl/traditional-models.pdf}
  \caption{Erreichte Accuracy-Werte und F1-Scores traditioneller ML-Modelle auf dem \acrshort{shl}-Datensatz mit 24 Features pro Sample.}\label{fig:shl-traditional-models}
\end{figure}

Die vielversprechendste Architektur lässt sich anhand dessen jedoch nicht zwangsweise vorhersagen, hierfür ist die Abhängigkeit des Modells von der Datenvorverarbeitung zu stark. Daher werden mehrere traditionelle Modelle auf den ermittelten Shallow Features des Datensatzes trainiert. Unter anderem wurde auch die von \cite{matusek_anwendung_2019} verwendete Architektur der Decision Trees und des \acrshort{mlp} getestet. Auf den Shallow Features konnten die traditionelle Verfahren einen F1-Score über dem Testdatensatz von maximal $70,42\%$ sowie eine Accuracy von $70,32\%$ ohne Postprocessing erreichen. Im Rahmen der \acrshort{shl}-Challenge würde sich diese Ergebnisqualität auf Platz 5 von 16 einordnen \cite[S. 3]{wang_summary_2020}. Das \acrshort{mlp} ergibt sich aus der Analyse als das am besten klassifizierende Modell, während die Speichergröße des \acrshort{mlp} auch gleichzeitig wesentlich unter dem betrachteten \acrshort{knn}- und \acrshort{svm}-Modell liegt.

\subsection{Test von Deep-Learning-Modellen}

\subsubsection{Problematik händisch erstellter Features}

Die Ermittlung von Shallow Features ist mit mehreren Problemen verbunden. Da die Features über jeden Datenpunkt für jedes Segment berechnet werden müssen, ist die Energieintensität der Vorverarbeitung gleichzeitig von der Art und Menge der Features abhängig. Mit Hinblick auf den Ressourcenverbrauch wurde zum Test der traditionellen Modelle daher die Anzahl der Shallow Features auf 24 aussagekräftige Werte beschränkt (\nameref{qa:r}). Auch hinsichtlich der Portierbarkeit (\nameref{qa:p}) stellt eine größere Anzahl von Features ein Problem dar. Während zur Selektion der Features bereits automatisierte Frameworks wie \cite{christ_time_2018} verwendet werden können, ist deren Portierung mit einem vergleichsweise hohen Aufwand verbunden, da bisher keine Möglichkeit existiert, die Feature-Pipelines direkt auf Smartphones zu portieren, ohne die Feature-Berechnungen nachzuimplementieren. Für einfache statistische Features ist dies denkbar, in komplexere Features müsste hingegen großer Zeitaufwand durch Einarbeitung in die mathematischen Grundlagen, Implementation und das Unit-Testing zur Verifikation der korrekten Funktionsweise fließen.

\subsubsection{Im ML-Modell inkludierte Feature-Extraktion}

Vor diesem Hintergrund scheint es attraktiv, statt Shallow Features händisch zu erstellen, die Zeitliniendaten direkt durch Deep-Learning-Modelle klassifizieren zu lassen. Dessen Plausibilität wurde durch verwandte Arbeiten gezeigt, und der Aufwand der Portierung würde um ein Vielfaches reduziert, während die Robustheit der Portierung gegenüber Softwarefehlern profitiert. Um dies näher zu betrachten, werden verschiedene Deep-Learning-Modelle, wieder prototypisch, jedoch nun direkt auf den Zeitliniendaten getestet. Die Auswahl kann nicht alle möglichen Architekturen einbeziehen, soll jedoch auch nicht ausschließlich arbiträr erfolgen, sondern bestimmten Heuristiken folgen. Hierzu gehört, dass die initiale Auswahl der Architekturen nach dem Schema einer Breitensuche durchgeführt werden soll, um sich noch nicht zu stark auf einen konkreten Ansatz zu konzentrieren. Dies kann realisiert werden, indem eine breite Auswahl von Grundarchitekturen abgetastet wird. Hierzu werden typische Architekturen aus verwandten Arbeiten, vor allem den Movebis-Ansätzen, wiederverwendet. Unter Umständen kann es auch förderlich sein, auch neuartige Architekturen in die Breitensuche mit einzubeziehen.

\subsubsection{Auswahl der zu testenden Basisarchitekturen}

Mit Hinblick auf die Movebis-Architekturen werden zunächst Basisarchitekturen getestet, welche aus diesen Arbeiten entspringen. Hierbei ist zunächst die \acrshort{lstm}-Basisarchitektur relevant, von der im Movebis-Ansatz \cite{werner_kontinuierliche_2020} eine Variation verwendet wurde. Als \acrshort{lstm} wird ein einschichtiges Modell mit Dropout-Regularisierung auf direkten Zeitliniendaten getestet. Außerdem wird die \acrshort{cnn}-Basisarchitektur, wie sie in \cite{stojanov_continuous_2020} verwendet wurde, getestet. Gewählt wird ein einfaches \acrshort{cnn} mit \acrshort{relu}-Aktivierungsfunktion und Regularisierung durch Batch Normalization. Das \acrshort{cnn} besteht aus drei Convolution-Schichten wie in \cite{stojanov_continuous_2020}, analog zu \cite{wang_time_2016} wird jedoch kein intermediäres Max-Pooling durchgeführt, sondern ein Global Average Pooling zum Schluss der drei Convolution-Schichten. Auf Grundlage der \acrshort{cnn}-Basisarchitektur aus \cite{stojanov_continuous_2020} wird eine weitere Basisarchitektur verwendet, das ResNet. Dieses substituiert die drei Convolution-Schichten wie im Movebis-Ansatz \cite{stojanov_continuous_2020} durch drei Convolution-Blöcke mit je drei Convolution-Schichten, welche untereinander durch Shortcut-Verbindungen ein tieferes Modell ermöglichen sollen, als es mit einem herkömmlichen \acrshort{cnn} möglich wäre \cite{he_deep_2015, ismail_fawaz_deep_2019}. Da aus der Forschungsanalyse hervorgegangen ist, dass tiefere Modelle bei der Verkehrsmittelerkennung bessere Ergebnisse erzeugen, könnte somit eine bessere Analyse der Tiefe erfolgen. Zu den bereits selektierten \acrshort{cnn}-Architekturen und einer \acrshort{lstm}-Architektur werden außerdem noch weitere neuartige Architekturen hinzugezogen. Hierzu gehört ein \acrshort{cnn}-\acrshort{lstm}. Dies ist ein \acrshort{lstm}, welches einem einschichtigen \acrshort{cnn} mit \acrshort{relu}-Aktivierungsfunktion zur Kodierung der Eingabewerte nachgeschaltet ist. Das \acrshort{cnn} übernimmt hierbei also die dedizierte Aufgabe der Feature-Codierung. Alle \acrshort{cnn}- und \acrshort{lstm}-Modelle verwenden eine Dense-Schicht als Ausgabeschicht, welche mithilfe der Softmax-Aktivierungsfunktion die kategorische Klassifikation übernimmt. Als neuartiger Ansatz wird außerdem ein Transformer-Netzwerk getestet, welches auf einem neuartigen Verfahren des Lernens basiert, welches als \textit{Attention}-based Learning bezeichnet wird \cite{vaswani_attention_2017, theodoros_ntakouris_keras_2021}.

\subsubsection{Hyperparameter des prototypischen Trainingsprozesses}

Beim Training der gewählten Deep-Learning-Modelle werden folgende Regeln angewandt. Die Modelle werden in die zuvor erläuterte Vorverarbeitungspipeline eingebunden und auf Batches mit einer Größe von 32 Samples (\textit{Batch-Size}) durch Supervised Learning trainiert, die Samples werden einem Shuffling unterzogen. Die Batch-Size von 32 hat sich für die gewählten Modelle auf dem Testsystem als performantere Konfiguration gegenüber größeren Batch-Sizes von 64 oder 128 erwiesen. Die Validierung des Modells geschieht auf einem separierten Datensatz. Als Variante des Gradientenabstiegsverfahrens wird der Adam-Optimizer genutzt. Als Alternativen stehen unter anderem der SGD-Optimizer und der RMSprop-Optimizer zur Verfügung. Der Adam-Optimizer zeigte im prototypischen Trainingsprozess gegenüber den anderen Optimierungsverfahren keine signifikanten Vor- und Nachteile, bis auf eine marginal schnellere Konvergenz. Als zugrundeliegende Verlustfunktion wird die \textit{Sparse Categorical Cross Entropy} verwendet, ein Derivat der Kreuzentropie, welche die Notwendigkeit eines One-Hot-Encodings der Labels beim Training der Modelle erübrigt. Da die Labels des \acrshort{shl}-Datensatzes in Form von Integer-Encodings vorliegen, muss somit keine weitere Vorverarbeitung auf den Labels erfolgen. Auf Grundlage der Validierungsmetriken wird Early Stopping verwendet, sowie eine automatisierte Reduktion der Learning Rate bei einer stagnierenden Verbesserung der Accuracy auf den Validierungsdaten.

\begin{figure}[h]
  \includegraphics[width=\linewidth, bb=0 0 856 207]{shl/deep-learning-models.pdf}
  \caption[Erreichte Accuracy-Werte von Deep-Learning-Modellen auf Zeitliniendaten des \acrshort{shl}-Datensatzes.]{Erreichte Accuracy-Werte von Deep-Learning-Modellen auf Zeitliniendaten des \acrshort{shl}-Datensatzes. Legende: Accuracy auf Trainingsdaten (obere Linie), Accuracy auf Testdaten (untere Linie), bestes Ergebnis traditioneller Modelle auf händischen Features (horizontale Gerade), Early Stopping (vertikale Gerade).}\label{fig:shl-deep-learning-models}
\end{figure}

Die besten Ergebnisqualitäten wurden hiermit durch das \cite{stojanov_continuous_2020} erweiternde ResNet-Modell und das auf der Basisarchitektur aus \cite{werner_kontinuierliche_2020} fundierende \acrshort{lstm}-Modell erreicht. Beide Modelle erreichen auf Zeitliniendaten bessere Ergebnisqualitäten als das \acrshort{mlp} analog zu \cite{matusek_anwendung_2019} auf Shallow Features. Die Trainingsverläufe sind in \Cref{fig:shl-deep-learning-models} dargestellt. Aus den Verläufen lassen sich mehrere wertvolle Informationen ableiten. Das bezüglich der Ergebnisqualität beste Modell ist das ResNet-\acrshort{cnn}. Hierbei ist es unerheblich, ob auf Vektorbeträgen oder auf allen Zeitlinien trainiert wird. Während das ResNet auf den Vektorbetragsdaten wesentlich kürzer zum Training für eine Epoche als das \acrshort{lstm} benötigt, trainiert es auch insgesamt für weniger Epochen bis zum Erreichen der bestmöglichen Ergebnisqualität in dieser Konfiguration. Die erreichte Accuracy von $78,94\%$ ist wesentlich besser als die des besten traditionellen Modells. Dennoch zeigt sich ein Overfitting auf den Trainingsdaten, auf welchen zeitweise mehr als $93,5\%$ Accuracy erreicht werden konnte.

\subsection{Selektion der ResNet-Architektur}

\begin{figure}[h]
  \includegraphics[width=\linewidth, bb=0 0 514 241]{resnet.pdf}
  \caption[Initiale Konfiguration eines residualen Netzwerks (ResNet) als Basisarchitektur.]{Initiale Konfiguration eines residualen Netzwerks (ResNet) als Basisarchitektur. Input ist ein Sample mit 500 Datenpunkten und 3 Sensordimensionen $\left| ACC \right|$, $\left| MAG \right|$ und $\left| GYR \right|$. Das \acrshort{cnn} besteht aus 3 Blöcken mit je 3 Schichten aus Convolution, Batch Normalization und \acrshort{relu}-Aktivierungsfunktion. Die Größe des Convolution-Kernels ist in jedem Block die Abfolge 8, 5, 3. Zwischen den Blöcken sind Shortcuts integriert, sogenannte \textit{Residual Connections}, welche sich an der biologischen Komposition pyramidaler Neuronen orientieren. Zur Klassifikation wird nach dem letzten Block ein \textit{Global Average Pooling} durchgeführt und mithilfe einer Dense-Schicht und der Softmax-Aktivierungsfunktion in die Verkehrsmittelklassen überführt.}\label{fig:resnet}
\end{figure}

Die Accuracy der in \Cref{fig:resnet} gezeigten ResNet-Architektur analog zum \acrshort{cnn} aus \cite{stojanov_continuous_2020}, hierbei jedoch direkt auf den Vektorbeträgen der Zeitliniendaten von $\left| ACC \right|$, $\left| MAG \right|$ und $\left| GYR \right|$ ist im prototypischen Vergleich mit den besten traditionellen Modellen und anderen Deep-Learning-Modellen als signifikant höher festzustellen. Es lässt sich feststellen, dass eine Klassifikation der Zeitliniendaten gegenüber einer Erstellung von Shallow Features auch vor dem Hintergrund dessen Problematiken für die Portierung hinreichende Ergebnisse verspricht. Obwohl \acrshort{lstm}-Netzwerke analog zu \cite{werner_kontinuierliche_2020} für die Verarbeitung von entlang einer Zeitlinie verteilten Daten konzeptioniert wurden, konnte das ResNet-Modell analog zu \cite{stojanov_continuous_2020} die Ergebnisse der getesteten \acrshort{lstm}-Netzwerke übertreffen. Darüber hinaus trainiert es anhand der Tests auch wesentlich schneller und benötigt weniger Epochen für das Erreichen der bestmöglichen Ergebnisqualität. Dies ist mit Hinblick auf Schritt 2 des Konzepts (\Cref{fig:prozedur}), welcher eine Rastersuche inkludiert, förderlich, da auf diese Weise in derselben Zeit mehr Modellvariationen evaluiert werden können. Insbesondere ist hierbei eine stärkere Regularisierung des ResNet attraktiv, welche das beobachtete Overfitting reduzieren und gleichzeitig die bisher noch nicht betrachteten Ressourcenparameter verbessern kann. Auch mit Hinblick auf Schritt 3 des Konzepts bieten sich generell bei \acrshort{cnn}-Architekturen attraktive Möglichkeiten zur Optimierung, wie die Einführung von separierbaren Convolution-Schichten, welche sich bei Verwendung von \acrshort{lstm}-Netzwerken nicht ergeben. Daher wird die ResNet-Architektur auf empirischer Grundlage als Basisarchitektur für die weiteren Betrachtungen selektiert.
...
\begin{figure}[h]
  ...
  \caption{\acrshort{stft}-Repräsentation des Eingabefrequenzspektrums von $\left| ACC \right|$, $\left| MAG \right|$ und $\left| GYR \right|$.}\label{fig:stft-layer-output}
\end{figure}

Darüber hinaus können auch die eindimensionalen Convolution-Schichten der Basisarchitektur auf zweidimensionale Convolution-Schichten übertragen werden. Dies wird in Betracht gezogen, da ResNets ursprünglich nicht für die Verarbeitung von 1D-Sequenzen entwickelt wurden, sondern zur Klassifikation von Bildern in Form von zweidimensionalen RGB- bzw. BGR-Pixeldaten. Hierfür wird eine portierbare \acrshort{stft}-Transformationsschicht (\textit{Short-Time-Fourier-Transform}) vorgeschaltet, deren Ausgabe analog zu \Cref{fig:stft-layer-output} als Eingabe für diese Architekturen dienen können. Dies fundiert auf den Beobachtungen, dass eine Fourier-Transformation wertvolle Informationen über den Signalverlauf entschlüsseln kann. Im Zuge dessen könnten auch auf dem Imagenet-Datensatz aus \cite{deng_imagenet_2009} vortrainierte ResNet-Modelle (ResNet50, ResNet101, ResNet152) in die Rastersuche einbezogen werden, um Transfer Learning als konzeptuellen Bestandteil mit einzubeziehen und potenziell von den bereits erlernten visuellen Features zu profitieren.

\subsection{Metamodell-Optimierung}\label{sec:metamodell-optimierung}
...
\begin{table}[h]
  ...
  \midrule
  Modelltyp
    & Input-Dimensionalität und Vorschaltung einer \acrshort{stft}-Schicht
    & 1D, 2D mit \acrshort{stft} \\
  Version
  ...
  Aktivierungsfunktion
    ...
    & \acrshort{relu} (Softmax für letzte Dense-Schicht) \\
  Regularisierung
    & Intermediäre Regularisierung zwischen Convolution-Schichten
    & Kein Dropout, keine L2/L1-Kernel-Regularisierung, jedoch Batch-Normalization und Layer-Normalization nach \acrshort{stft} (falls existent) \\
  \midrule
  ...
  Early Stopping
    ...
    & Nach drei Epochen ohne Verbesserung \\
  Learning Rate Decay
    & Automatische Reduktion der Learning Rate
    & Halbierung nach einer Epoche ohne Verbesserung \\
  \bottomrule
  \end{tabular}%
  }
...
Das deklarierte Metamodell ermöglicht die Erzeugung von ResNet-Modellen für den \acrshort{shl}-Datensatz, indem die Hyperparameter eines Modells als Parameter in das Metamodell eingegeben werden. Die genauen Hyperparameter-Variationsmöglichkeiten sind in \Cref{tab:hyperparameter-suchraum} dargestellt. Zusätzlich wird die Batch-Size gegenüber dem Prototyping von 32 auf 64 angehoben, um eine optimale Auslastung der GPU zu ermöglichen und das Rauschen des Gradientenabstiegsverfahrens zu reduzieren\footnote{Durch die Inklusion wesentlich größerer Modelle bei der Rastersuche als bei der Selektion der Basisarchitektur ist es möglich, dass die GPU das Training auf einer Batch-Size von 128 durch Überlauf des VRAM nicht durchführen kann.}. Anschließend können die erzeugten Modelle trainiert und bezüglich deren Accuracy getestet werden.
...
Zur Messung des Energieverbrauchs wird die nach \Cref{sec:vorbedingungen} erstellte App für eine Dauer von mehreren Minuten auf dem Smartphone ausgeführt. Gleichzeitig wird ein externer Profiler eingekoppelt oder die Smartphone-eigenen Fähigkeiten zur Aufzeichnung der Energieintensität genutzt. Da das Profiling auf iOS-Geräten stattfindet, ist es nicht möglich, wie auf Android-Geräten den direkten Akkuverbrauch in SI-Einheiten zu schätzen. Stattdessen stellt Apple eine eigene Einheit zur Verfügung, die sogenannten \textit{Abstract Energy Units} (\acrshort{aeu}), welche in diskreten Schritten von 0, 1, 2 bis 20 reichen. Für diese Einheit stehen jedoch nach aktuellem Stand nur sehr wenige Informationen zur Verfügung\footnote{\url{https://developer.apple.com/library/archive/documentation/Performance/Conceptual/EnergyGuide-iOS/MonitorEnergyWithInstruments.html} (Abgerufen am 7.9.2021)}. Vor diesem Hintergrund lassen sich die Modelle zwar in Relation setzen, es ist jedoch nicht bekannt, ob die Skala der \acrshort{aeu} beispielsweise linear, polynomiell oder logarithmisch ist. Daher wird insbesondere die CPU-Auslastung als weiterer Parameter hinzugezogen. Die CPU-Auslastung kann neben dem Verbrauch des Arbeitsspeichers sowohl durch einen externen Profiler gemessen werden, oder automatisiert durch hardwarenahe Schnittstellen durch Unit-Tests ähnlich zur Inferenzzeit. Beide Methoden werden zur Ermittlung der Tradeoff-Parameter herangezogen.
...
Das Quantization Aware Training (\acrshort{qat}) bietet anschließend die Möglichkeit, ein Modell nach dessen Quantisierung einem weiteren Fine-Tuning zu unterziehen, um eventuelle Accuracy-Verluste auszugleichen. Um auf die Fortschritte des Prunings aufzubauen, kann eine spezielle Form des \acrshort{qat} verwendet werden, das Sparsity Preserving \acrshort{qat} (\acrshort{pqat})\footnote{\url{https://www.tensorflow.org/model_optimization/guide/combine/pqat_example} (Abgerufen am 7.9.2021)}.
...
Als Datensatz wird der \acrshort{shl}-Forschungsdatensatz akquiriert. Auf diesem Datensatz wurden traditionelle Modelle und Deep-Learning-Modelle prototypisch getestet, unter Einbeziehung der Basisarchitekturen aus den Movebis-Ansätzen \cite{werner_kontinuierliche_2020,stojanov_continuous_2020,matusek_anwendung_2019}, insbesondere \acrshort{dt}, \acrshort{mlp}, \acrshort{lstm} und \acrshort{cnn}. Dies inkludiert individuell angepasste Datenvorverarbeitungspipelines, welche empirisch durch eine Datensatzanalyse konfiguriert wurden. Dazu gehört, dass neben einem speichereffizienten Lademechanismus die im Datensatz enthaltenen Klassen durch \acrshort{smote} oder die Gewichtung der Modell-Verlustfunktion wie in \cite{werner_kontinuierliche_2020} gleichgewichtet werden. Außerdem werden die Datenpunkte analog zu \cite{werner_kontinuierliche_2020} in eine annähernd symmetrische Repräsentation durch Yeo-Johnson-Transformation überführt. Das beste empirisch gefundene Basismodell ist eine Variante der ResNet-\acrshort{cnn}-Architektur, welche eine Erweiterung der \acrshort{cnn}-Basisarchitektur aus \cite{stojanov_continuous_2020} darstellt. Zur Elimination von Verschiebungsfaktoren wie der Gravitation werden die Sensorsignale des Akzelerometers, des Magnetometers und des Gyrosensors bei diesem Modell in Zeitlinien des jeweiligen Vektorbetrags überführt, ohne Anwendung eines filterbasierten \acrshort{ahrs}, wie es in \cite{matusek_anwendung_2019} vorgestellt wurde. Shallow Features und Non-Shallow Features wurden bei der Selektion des Basismodells ebenfalls mit einbezogen. Anhand von vorläufigen Tests und Betrachtungen der Portierbarkeit und der Ressourcenintensität wurden Modelle auf diesen Features nicht weiter verfolgt.
...
Anhand der portierten Modelle wird ein gerätespezifisches Profiling durchgeführt. Hierfür werden konkrete experimentelle Vorbedingungen eingeführt. Die Ressourcenparameter Inferenzzeit, CPU-Auslastung und RAM-Auslastung können anschließend durch Unit-Tests und in Kooperation mit einem externen Profiler ermittelt werden. Die Messung der Energieintensität ist auf iOS wegen der Limitierung der Systemschnittstellen auf die Einbindung eines externen Profilers angewiesen. Die resultierenden Tradeoff-Parameter der Top-N-Modelle werden in einer hierachische Tradeoff-Analyse evaluiert. Auch hier wird der Aufwand reduziert, indem zunächst eine simple statistische Analyse durchgeführt wird, um anschließend, falls auf Grundlage dessen als notwendig erachtet, eine selektive Priorisierung der Tradeoff-Parameter vorzunehmen oder gegebenenfalls ein regressives Metamodell zu erstellen. Das resultierende Modell soll einem hypothetisch Pareto-optimalen Modell möglichst nah kommen. Das resultierende Modell wird schließlich weiter optimiert, durch Fine-Tuning-Optimierung über Pruning und \acrshort{pqat}, um eine abschließende Betrachtung der optimierten Variante im Vergleich zur nicht optimierten Variante vorzunehmen.
