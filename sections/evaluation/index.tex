...
Ein eng mit der Inferenzzeit verknüpfter Ressourcenparameter bildet sich in der CPU-Auslastung. Wird das Modell ausschließlich auf der CPU ausgeführt, korreliert die CPU-Auslastung mit der Inferenzzeit sehr stark positiv ($R_{iPhoneSE}\approx0.98$, $R_{iPhoneXS}\approx0.995$). Bei der Auslagerung der Modelle auf Hardwarebeschleuniger verzerrt sich dieses Bild, je nachdem, in welchem Anteil die Modelle noch auf der CPU ausgeführt werden. Das bereits häufiger als Beispiel genannte Modell \texttt{752315} kann die CPU-Auslastung um bis zu 227\% reduzieren. Die Last wird jedoch nur auf die GPU verlagert. Ob dies in einem Vorteil im Energieverbrauch resultiert, muss also separat geprüft werden.
